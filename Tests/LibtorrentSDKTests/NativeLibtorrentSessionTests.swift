    ...
    final class NativeLibtorrentSessionTests: XCTestCase {
        ...
        func testNativeSessionDispatchesProgressFromNativeABI() async throws {
            ...
            case let .progress(progress):
                ...
                XCTAssertEqual(progress.swarmDiagnostics?.dht?.nodeCount, 42)
                XCTAssertEqual(progress.swarmDiagnostics?.portMappings.first?.status, "mapped")
            default:
